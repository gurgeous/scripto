...
class TestPrint < Minitest::Test
  ...
  def test_loud
    Scripto.verbose!
    assert_output(/gub/) { Scripto.vbanner "gub" }
    assert_output(/gub/) { Scripto.vprintf("zub %s", "gub") }
    assert_output(/gub/) { Scripto.vputs "gub" }
  end

  def test_warning
    assert_output(/gub/) { Scripto.warning "gub" }
  end
end
